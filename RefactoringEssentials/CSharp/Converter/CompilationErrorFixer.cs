...
namespace RefactoringEssentials.CSharp.Converter
{
    internal class CompilationErrorFixer
    {
        ...
        private SyntaxNode ComputeReplacementNode(SyntaxNode originalNode, SyntaxNode potentiallyRewrittenNode)
        {
            ...
            var methodSymbol = semanticModel.GetSymbolInfo(invocationExpression).CandidateSymbols.OfType<IMethodSymbol>().FirstOrDefault(s => invocationExpression.ArgumentList.Arguments.Count == s.Parameters.Length);
